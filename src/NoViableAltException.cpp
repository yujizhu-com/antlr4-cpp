...
CppRef<atn::ATNConfigSet> buildConfigsRef(atn::ATNConfigSet *configs, bool deleteConfigs) {
  if (deleteConfigs) {
    return CppRef<atn::ATNConfigSet>(configs);
  } else {
    return CppRef<atn::ATNConfigSet>(configs, [](atn::ATNConfigSet *){});
  }
}
